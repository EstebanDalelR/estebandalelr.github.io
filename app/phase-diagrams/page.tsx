...
type Pt = [number, number]; // [composition, temperature °C]
type Region = { id: string; phases: string[]; poly: Pt[]; bounds?: [Pt[], Pt[]]; label: Pt; callout?: Pt };
type Invariant = { kind: string; T: number; x: number; line: [number, number]; reaction: string; comps: { phase: string; x: number }[] };
type View = { label: string; x0: number; x1: number; T0: number; T1: number; xStep: number; TStep: number };
type Micro = { title: string; rows: { name: string; frac: number }[] };
type Diagram = {
  id: string; name: string; xLabel: string; unit: string; xDigits: number; blurb: string; start: Pt;
  views: View[]; regions: Region[]; invariants: Invariant[]; phases: Record<string, string>;
  classify: (x: number) => string | null; micro: (x: number) => Micro | null;
};
type Row = { phase: string; x: number; w: number };
type State = { region: Region | null; rows: Row[]; tie: [number, number] | null };
type CoolEvent = { T: number; from: string; to: string; inv: Invariant | null };

// ---- ENGINE START
const reg = (phases: string[], poly: Pt[], label: Pt, bounds?: [Pt[], Pt[]], callout?: Pt): Region => ({ id: phases.join(' + '), phases, poly, label, bounds, callout });
const rev = (p: Pt[]): Pt[] => [...p].reverse();

/* ---------- Cu–Ni: isomorphous ---------- */
const cuLiq: Pt[] = [[0, 1085], [10, 1140], [20, 1195], [30, 1245], [40, 1282], [50, 1320], [60, 1360], [70, 1395], [80, 1425], [90, 1445], [100, 1455]];
const cuSol: Pt[] = [[0, 1085], [10, 1100], [20, 1135], [30, 1175], [40, 1240], [50, 1280], [60, 1312], [70, 1350], [80, 1390], [90, 1425], [100, 1455]];

/* ---------- Pb–Sn: eutectic ---------- */
const pbLiqA: Pt[] = [[0, 327], [15, 295], [30, 260], [46, 220], [61.9, 183]];
const pbSolA: Pt[] = [[0, 327], [8, 280], [13, 250], [17, 220], [18.3, 183]];
const pbSolvA: Pt[] = [[18.3, 183], [11, 150], [7, 100], [4, 50], [2, 0]];
const pbLiqB: Pt[] = [[61.9, 183], [80, 205], [100, 232]];
const pbSolB: Pt[] = [[97.8, 183], [100, 232]];
const pbSolvB: Pt[] = [[97.8, 183], [99, 150], [99.5, 50], [99.7, 0]];

/* ---------- Fe–Fe3C: metastable iron–carbon ---------- */
const feLiqD: Pt[] = [[0, 1538], [0.53, 1493]];
const feSolD: Pt[] = [[0, 1538], [0.09, 1493]];
const feDG: Pt[] = [[0.09, 1493], [0, 1394]];
const feGD: Pt[] = [[0.17, 1493], [0, 1394]];
const feLiqG: Pt[] = [[0.53, 1493], [4.3, 1147]];
const feSolG: Pt[] = [[0.17, 1493], [2.14, 1147]];
const feLiqC: Pt[] = [[4.3, 1147], [6.7, 1227]];
const feAcm: Pt[] = [[0.76, 727], [2.14, 1147]];
const feA3: Pt[] = [[0, 912], [0.76, 727]];
const feA3a: Pt[] = [[0, 912], [0.022, 727]];
const feSolvA: Pt[] = [[0.022, 727], [0.005, 400]];
const feCem: Pt[] = [[6.7, 400], [6.7, 1600]];

const DIAGRAMS: Diagram[] = [
  {
    id: 'cuni', name: 'Cu–Ni', xLabel: 'Composition (wt% Ni)', unit: 'wt% Ni', xDigits: 1, start: [35, 1250],
    ...
    views: [{ label: 'Full', x0: 0, x1: 100, T0: 1000, T1: 1500, xStep: 10, TStep: 100 }],
    phases: {
    ...
    regions: [
      ...
      reg(['α'], [...cuSol, [100, 1000], [0, 1000]], [60, 1120]),
    ],
    invariants: [],
    classify: () => null,
    micro: () => null,
  ...
  {
    id: 'pbsn', name: 'Pb–Sn', xLabel: 'Composition (wt% Sn)', unit: 'wt% Sn', xDigits: 1, start: [40, 150],
    ...
    phases: {
      ...
      'α': 'Pb-rich FCC solid solution. Dissolves at most 18.3 wt% Sn, at 183 °C; solubility drops on cooling (solvus).',
    ...
    regions: [
      ...
      reg(['β'], [...rev(pbSolB), ...pbSolvB.slice(1), [100, 0]], [99.4, 110], undefined, [91, 125]),
    ...
    classify: (x) => {
      ...
      if (x < 61.9) return 'Hypoeutectic: primary α forms first, remaining liquid becomes eutectic at 183 °C.';
      if (x < 97.8) return 'Hypereutectic: primary β forms first, remaining liquid becomes eutectic at 183 °C.';
      return 'No eutectic forms. Freezes to β, with α precipitating below the solvus.';
    },
    micro: (x) => {
      const title = 'Microconstituents just below 183 °C';
      if (x > 18.3 && x < 61.9) { const we = (x - 18.3) / (61.9 - 18.3); return { title, rows: [{ name: 'Primary α', frac: 1 - we }, { name: 'Eutectic (α + β lamellae)', frac: we }] }; }
      if (x >= 61.9 && x < 97.8) { const we = (97.8 - x) / (97.8 - 61.9); return { title, rows: [{ name: 'Eutectic (α + β lamellae)', frac: we }, { name: 'Primary β', frac: 1 - we }] }; }
      return null;
    },
  },
  {
    id: 'fec', name: 'Fe–Fe₃C', xLabel: 'Composition (wt% C)', unit: 'wt% C', xDigits: 3, start: [0.4, 800],
    ...
    regions: [
      ...
      reg(['γ'], [[0, 1394], [0.17, 1493], [2.14, 1147], [0.76, 727], [0, 912]], [0.6, 1000]),
      ...
      reg(['α'], [[0, 912], [0.022, 727], [0.005, 400], [0, 400]], [0.008, 650]),
      reg(['α', 'Fe₃C'], [[0.022, 727], [6.7, 727], [6.7, 400], [0.005, 400]], [1.5, 600], [feSolvA, feCem]),
    ],
    invariants: [
    ...
    classify: (x) => {
      ...
      if (Math.abs(x - 0.76) <= 0.01) return 'Eutectoid steel: transforms fully to pearlite at 727 °C.';
      ...
      if (Math.abs(x - 4.3) <= 0.02) return 'Eutectic cast iron: liquid freezes directly to ledeburite at 1147 °C.';
    ...
    micro: (x) => {
      if (x > 0.022 && x < 0.76) { const wp = (x - 0.022) / (0.76 - 0.022); return { title: 'Microconstituents just below 727 °C', rows: [{ name: 'Proeutectoid ferrite', frac: 1 - wp }, { name: 'Pearlite (α + Fe₃C)', frac: wp }] }; }
      if (x >= 0.76 && x < 2.14) { const wp = (6.7 - x) / (6.7 - 0.76); return { title: 'Microconstituents just below 727 °C', rows: [{ name: 'Pearlite (α + Fe₃C)', frac: wp }, { name: 'Proeutectoid cementite', frac: 1 - wp }] }; }
      if (x >= 2.14 && x < 4.3) { const we = (x - 2.14) / (4.3 - 2.14); return { title: 'Microconstituents just below 1147 °C', rows: [{ name: 'Primary austenite', frac: 1 - we }, { name: 'Ledeburite (γ + Fe₃C)', frac: we }] }; }
      if (x >= 4.3 && x < 6.7) { const we = (6.7 - x) / (6.7 - 4.3); return { title: 'Microconstituents just below 1147 °C', rows: [{ name: 'Ledeburite (γ + Fe₃C)', frac: we }, { name: 'Primary cementite', frac: 1 - we }] }; }
      return null;
    },
  },
];

const lerpX = (poly: Pt[], T: number): number | null => {
...
const regionAt = (d: Diagram, x: number, T: number): Region | null => {
  const v = d.views[0];
...
const stateAt = (d: Diagram, x: number, T: number): State => {
...
const coolingPath = (d: Diagram, x: number): CoolEvent[] => {
  const v = d.views[0];
  const N = 1500, dt = (v.T1 - v.T0) / N;
  const key = (T: number) => regionAt(d, x, T)?.id ?? '—';
  const ev: CoolEvent[] = [];
  ...
  for (let i = 1; i <= N; i++) {
    ...
    if (k !== prev) {
      let hi = prevT, lo = T;
      for (let j = 0; j < 32; j++) { const m = (hi + lo) / 2; if (key(m) === prev) hi = m; else lo = m; }
      const Tb = (hi + lo) / 2;
...
const COLOR: Record<string, string> = { L: '#ff8a7a', 'α': '#7fe3c6', 'β': '#9db4ff', 'γ': '#ffc15e', 'δ': '#e39bff', 'Fe₃C': '#e6dcc6' };
const W = 720, H = 500, ML = 66, MR = 18, MT = 18, MB = 54;
const PW = W - ML - MR, PH = H - MT - MB;
const clamp = (v: number, a: number, b: number) => Math.min(b, Math.max(a, v));
const pct = (w: number) => (Number.isFinite(w) ? `${(w * 100).toFixed(1)}%` : '—');
const tickFmt = (v: number, step: number) => (step < 1 ? v.toFixed(1) : String(Math.round(v)));

export default function PhaseDiagramExplorer() {
  const [di, setDi] = useState(0);
  const [vi, setVi] = useState(0);
  const [pt, setPt] = useState<Pt>(DIAGRAMS[0].start);
  ...
  const v = d.views[vi];
  const [x, T] = pt;
  const sx = (cx: number) => ML + ((cx - v.x0) / (v.x1 - v.x0)) * PW;
  const sy = (cT: number) => MT + ((v.T1 - cT) / (v.T1 - v.T0)) * PH;

  const st = useMemo(() => stateAt(d, x, T), [d, x, T]);
  const path = useMemo(() => coolingPath(d, x), [d, x]);
  const fullRange = d.views[0].T1 - d.views[0].T0;
  const nearInv = d.invariants.find((q) => Math.abs(q.T - T) <= fullRange * 0.006 && x >= q.line[0] && x <= q.line[1]) ?? null;
  const cls = d.classify(x);
  const micro = d.micro(x);
  const fx = (cx: number) => cx.toFixed(d.xDigits);

  const pickDiagram = (i: number) => { setDi(i); setVi(0); setPt(DIAGRAMS[i].start); setPhaseSel(null); };

  const setFromPointer = (e: RPointerEvent<SVGSVGElement>) => {
  ...
  const inView = (cx: number, cT: number) => cx >= v.x0 && cx <= v.x1 && cT >= v.T0 && cT <= v.T1;
    ...
    >
      ...
      <g clipPath="url(#pd-clip)">
        {d.regions.map((r) => {
          ...
          return (
            <g key={r.id} opacity={hit ? 1 : 0.3}>
              {r.phases.length === 1
                ? <polygon points={pts} fill={COLOR[r.phases[0]]} fillOpacity={phaseSel && hit ? 0.34 : 0.2} />
                : <>
        ...
        {d.regions.map((r) => {
          ...
          if (r.callout) {
            return (
              <g key={`l${r.id}`} className="pd-label">
      ...
      {d.invariants.filter((q) => inView(q.x, q.T)).map((q) => (
        <g key={q.kind} className="pd-inv" onPointerDown={(e) => { e.stopPropagation(); setPt([q.x, q.T + 0.01]); }}>
    ...
    <div className="pd">
      ...
      <header className="pd-head">
        ...
        <div className="pd-tabs" role="tablist">
          {DIAGRAMS.map((D, i) => (
            <button key={D.id} role="tab" aria-selected={i === di} className={`pd-tab ${i === di ? 'on' : ''}`} onClick={() => pickDiagram(i)}>{D.name}</button>
          ))}
        </div>
      </header>
        ...
        <div>
          ...
          <div className="pd-sliders">
            <label>
              ...
              <input type="range" min={v.x0} max={v.x1} step={d.xDigits === 3 ? 0.001 : 0.1} value={x} onChange={(e) => setPt([+e.target.value, T])} />
            </label>
            <label>
        ...
        <aside>
          <section className="pd-card">
            ...
            {d.views.length > 1 && (
              <div className="pd-row">
                {d.views.map((V, i) => (
                  <button key={V.label} aria-pressed={i === vi} className={`pd-btn ${i === vi ? 'on' : ''}`} onClick={() => { setVi(i); setPt([clamp(x, V.x0, V.x1), clamp(T, V.T0, V.T1)]); }}>{V.label}</button>
                ))}
              </div>
            )}
          </section>
          ...
          <section className="pd-card">
            ...
            {d.invariants.length === 0 ? (
              <p className="pd-small">None. An isomorphous system has no three-phase equilibria.</p>
            ) : (
              <div className="pd-row">
                {d.invariants.map((q) => (
                  <button key={q.kind} className="pd-btn" onClick={() => {
          ...
          <section className="pd-card">
            ...
            <div className="pd-row">
              {Object.keys(d.phases).map((p) => (
                <button key={p} aria-pressed={phaseSel === p} className={`pd-btn ${phaseSel === p ? 'on' : ''}`} onClick={() => setPhaseSel(phaseSel === p ? null : p)}>
